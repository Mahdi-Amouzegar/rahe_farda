...
## D-009 — محدودیت Legacy Telegram Login
...
### زمینه
در Redirect-based Telegram Login (`oauth.telegram.org/auth`)، هیچ پارامتر
رسمی یا API وب برای «اجبار به پرسیدن شماره تلفن در هر ورود» وجود ندارد.
خروج از حساب سایت ≠ خروج از session تلگرام.

تحقیقات (Stack Overflow + مستندات Telegram + تست عملی):
- پارامتر `logout=1` **کار نمی‌کند** (در Redirect-based پشتیبانی نمی‌شود).
- `nonce` در `return_to` بی‌اثر است.
- تغییر `origin` مجاز نیست (خطای `Bot domain invalid`).
- کوکی `oauth.telegram.org` از دامنه‌ی ما قابل پاک‌کردن نیست (Same-Origin Policy).
- صفحه‌ی `https://oauth.telegram.org/auth` **بدون پارامترهای اضافی** خطای
  `bot_id required` می‌دهد.
- صفحه‌ی `https://oauth.telegram.org/auth?...&logout=1` **دوباره فرایند Login
  را شروع می‌کند** (نه Logout) و چون session هست، بدون پرسیدن شماره تأیید می‌کند.
- تنها راه رسمی: کاربر در **اپ تلگرام** session را Terminate کند
  (Settings → Privacy and Security → Logged in with Telegram).

### تصمیم
- قبول این رفتار به‌عنوان طراحی رسمی تلگرام.
- در مودال «حساب من»، **دکمه‌ی دوم** اضافه می‌شود:
  «🔗 ورود با حساب تلگرام دیگر»
- این دکمه **مودال راهنما** (با `showInfoModal`) نشان می‌دهد که شامل
  ۶ مرحله‌ی گام‌به‌گام برای Terminate کردن session در اپ تلگرام است.
- ما نمی‌توانیم از کد، session را قطع کنیم.
- مهاجرت به **OIDC + PKCE** (روش رسمی جدید تلگرام) **به فاز ۷ موکول شد**.

### دلیل
- **OIDC هم مشکل خروج/ورود را حل نمی‌کند** — پارامتر رسمی `prompt=login`
  در OIDC وجود دارد ولی تلگرام در پیاده‌سازی خودش پشتیبانی نمی‌کند.
- پیاده‌سازی OIDC نیاز به Client Secret + JWKS + PKCE دارد (~۳-۵ ساعت کار).
- راهنمای متنی در ~۱۵ دقیقه مشکل UX را حل می‌کند.
- راهنمای متنی صادقانه‌تر است: کاربر می‌داند چرا باید در اپ تلگرام برود.
- فاز ۶ اهداف بزرگ‌تری دارد (Sync Engine، Sync Code، مدیریت حساب).

### تأثیر
- `js/app.js`: دکمه‌ی «ورود با حساب تلگرام دیگر» + مودال راهنما در `renderAuthModal`.
- `public/css/components.css`: استایل دکمه (بدون تغییر).
- `DECISIONS.md`: همین بلاک.
- فاز ۷ (اختیاری): مهاجرت به OIDC.

## D-010 — Sync ابری فقط با تأیید صریح + بدون ارسال خودکار لوکال به سرور

**تاریخ:** ۱۴۰۵/۰۶/۲۹ (2026-09-19)
**فاز:** ۶ — گام ۳ (Sync Engine) و گام ۴ (Sync Code)
**وضعیت:** ✅ فعال

### زمینه
کاربر ممکن است داده‌های محلی محرمانه داشته باشد. اگر به‌طور خودکار با
Login به سرور sync شود، همه‌ی دستگاه‌های آن حساب (که ممکن است شرکت،
خانواده، یا دوستان باشند) به اطلاعات محرمانه دسترسی پیدا می‌کنند.
همچنین «ارسال لوکال به سرور» خطر حذف کل سرور توسط اشتباه کاربر را دارد.

### تصمیم
- `state.sync.enabled = false` همچنان پیش‌فرض است (طبق D-004).
- کاربر باید در تنظیمات صریحاً «همگام‌سازی ابری» را فعال کند.
- در فعال‌سازی، اگر هر دو طرف (لوکال و سرور) داده دارند → **مودال تصمیم‌گیری**.
- **فقط دو گزینه:**
  1. 🗑 **حذف لوکال و استفاده از داده‌های سرور**
  2. ⏸ **انصراف — sync خاموش بماند**
- ❌ **گزینه‌ی «ارسال لوکال به سرور» حذف شد** (خطر حذف کل سرور).
- ❌ **گزینه‌ی «ادغام خودکار» حذف شد** (خطر حریم خصوصی + پیچیدگی conflict).
- **پشتیبان خودکار:** قبل از دانلود از سرور، اپ به‌طور خودکار یک فایل
  JSON از داده‌ی محلی می‌سازد و دانلود می‌کند (نام:
  `rahe-farda-autobackup-YYYYMMDD-HHMM.json`).
- **تأیید ثانویه:** قبل از هر عمل مخرب (حذف لوکال یا replace در Import)،
  یک مودال «مطمئنید؟» نمایش داده می‌شود.

### جریان ادغام دستی (راهنمای کاربر)
اگر کاربر می‌خواهد داده‌های محلی و سرور را با هم داشته باشد:
1. ابتدا **پشتیبان دستی** از لوکال بگیرد (تنظیمات → پشتیبان‌گیری).
2. sync ابری را فعال کند با گزینه‌ی «حذف لوکال».
3. داده‌های سرور دانلود و جایگزین شوند.
4. سپس از فایل پشتیبان، **Import** کند با گزینه‌ی «ادغام».

### Import (بازیابی از پشتیبان) — تصمیمات مرتبط
- مودال انتخاب «ادغام / جایگزینی» نمایش داده می‌شود (در گام ۵ فاز ۵ پیاده شد).
- **تأیید ثانویه** قبل از عمل (به‌خصوص برای «جایگزینی کامل»).
- در حالت «جایگزینی»، **پشتیبان خودکار** از داده‌ی فعلی گرفته شود.
- در حالت «ادغام»، تصمیم‌گیری برای task تکراری (نگه‌داشتن محلی یا سرور)
  به گام ۳ موکول می‌شود.

### دلیل
- **حریم خصوصی کاربر اولویت اول است.**
- هیچ داده‌ای بدون تأیید صریح به سرور نمی‌رود.
- «ارسال لوکال به سرور» با یک کلیک می‌تواند کل سرور را حذف کند → بسیار خطرناک.
- پشتیبان خودکار = تور نجات در صورت اشتباه.
- تأیید ثانویه = جلوگیری از اشتباه کاربر.
- مدل workspaceهای چندگانه پیچیده‌تر از حد فاز ۶ است.

### تأثیر
- `js/app.js` یا `js/settings-sync.js`: مودال تصمیم‌گیری + تأیید ثانویه (گام ۳).
- `js/sync-queue.js`: تابع `enableCloudSync` با بررسی اولیه (گام ۳).
- `js/store.js`: تابع `createAutoBackup()` (گام ۳).
- `index.html`: مودال جدید (گام ۳).
- Worker: endpoint `GET /api/sync/stats` (تعداد taskهای کاربر) (گام ۳).
- `js/app.js` (Import): افزودن تأیید ثانویه + پشتیبان خودکار (گام ۳ یا بعد).