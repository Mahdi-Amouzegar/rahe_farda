...
### زمینه
در Redirect-based Telegram Login (`oauth.telegram.org/auth`)، هیچ پارامتر رسمی
برای اجبار به پرسیدن شماره در هر ورود وجود ندارد. خروج از سایت ≠ خروج از session تلگرام.

### تصمیم
- قبول این رفتار به‌عنوان طراحی رسمی تلگرام.
- در modal «حساب من»، دکمه‌ی دوم «قطع اتصال از تلگرام» اضافه می‌شود.
- این دکمه کاربر را به `oauth.telegram.org/auth` می‌فرستد (صفحه‌ی Logout تلگرام).
- مهاجرت به OIDC + PKCE به فاز ۷ موکول می‌شود.

### دلیل
- عدم وجود پارامتر رسمی `force_login` / `force_phone`.
- Same-Origin Policy اجازه‌ی پاک کردن cookie تلگرام از دامنه‌ی ما را نمی‌دهد.
- OIDC نیاز به بازنویسی بزرگ دارد (خارج از محدوده‌ی فاز ۶).

### تأثیر
- `js/app.js`: افزودن دکمه‌ی دوم در `renderAuthModal`.
- `public/css/components.css`: استایل دکمه‌ی جدید.
- فاز ۷: مهاجرت به OIDC (اختیاری).
---
