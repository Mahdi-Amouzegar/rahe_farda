...
در نسخه `v1.3.1.0` تمرکز اصلی روی **نمایش آیکن وضعیت هوا روی کارت وظایف** بوده است — با کش پایدار در `localStorage` (TTL ۳ ساعت) برای جلوگیری از محدودیت نرخ درخواست Open-Meteo. همچنین تصحیحات فاز ۱+۲ شامل: افزودن `viewport-fit`، `structuredClone` fallback، حذف کد مرده، بهبود cache، guards در `weather-modal`، بازطراحی `child-item` برای نظم بصری، و بهبود جستجوی مکان با درخواست واحد دوزبانه از Nominatim.